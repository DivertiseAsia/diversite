...
let renderScene = (scene) => {
  if (!isNotSupportedWebGl()) {
    Document.setTimeout(() => {
      ...
      | Laser => {
          LaserScene.initScene(Document.offsetWidth(element), Document.offsetHeight(element));
          LaserScene.scene[0];
        }
      | Pentagon => {
      ...
      | Ball => {
          ...
          Document.set_mousemove(element, (e) => BallWorldScene.onMouseMove(e, Document.offsetWidth(element), Document.offsetHeight(element)) |> ignore);
...
let make = (_children: array(ReasonReact.reactElement)) => {
  ...
  reducer: (action, state) =>
    switch (action) {
      | ChangeScene(scene) => UpdateWithSideEffects(
        {...state, scene},
