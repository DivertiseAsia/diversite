...
module PentagonScene = {
  ...
  let allParticle = [list{}]
  let temp_particles_body = [list{}]

  let particles_body: list<particle> = list{
    {
      x: -.width /. 2. +. 120.,
      y: -.height /. 2. +. 120.,
      z: 0.,
  ...
  @set external set_o: (particle, float) => unit = "o"
  @set external set_angle: (particle, float) => unit = "angle"
  ...
  let buildParticle = () => {
    ...
    |> List.map(particle => {
      ...
      let params = Three.parseIntoParameters(
      ...
      temp_particles_body[0] = List.append(temp_particles_body[0], list{newBody})
      allParticle[0] = List.append(allParticle[0], list{line})
      Three.addLineToScene(scene[0], line)
    })
    |> ignore
  }
  let updateParticle = () => {
    let updated_particles_body = [list{}]
    allParticle[0]
    |> List.mapi((index, particle) => {
      ...
      Three.set_x(
        particle["position"],
        particle["position"]["x"] +. Js.Math.cos(body.angle *. Js.Math._PI /. 180.) *. 0.25,
      )
      Three.set_y(
        particle["position"],
        particle["position"]["y"] +. Js.Math.sin(body.angle *. Js.Math._PI /. 180.) *. 0.25,
      )
      let newBody = {
      ...
      updated_particles_body[0] = List.append(updated_particles_body[0], list{newBody})
    })
    |> ignore
