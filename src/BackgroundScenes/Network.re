...
module NetworkScene = {
  ...
  let createdPoint = (id, x, y, z, r, pinned) => {
    ...
    let params =
      Three.parseIntoParameters("{\"color\": \"" ++ {j|$color|j} ++ "\"}");
    let material = Three.meshBasicMaterial(params);
  ...
  [@bs.send]
  external set_distanceTo: (Three.position, Three.position) => float =
    "distanceTo";
  let makeEdge = (v1, v2) => {
    let p1 = points[0][v1];
    let p2 = points[0][v2];
    ...
    Three.geometry_vertices_pushs(
      geometry_line##vertices,
      p1.position,
      p2.position,
    );
  ...
  let buildObject = (width, height) => {
    ...
    for (row_index in 0 to (row |> int_of_float) - 1) {
      for (column_index in 0 to (col |> int_of_float) - 1) {
        let radius = radiusList[Js.Math.floor(
                                  Js.Math.random()
                                  *. (
                                    Array.length(radiusList) |> float_of_int
                                  ),
                                )];

        let x =
          -. xstart
          +. (column_index |> float_of_int)
          *. colWidth
          +. (
            column_index mod ((col |> int_of_float) - 1) !== 0
              ? Js.Math.random() *. rangeX : 0.
          );
        let y =
          ystart
          -. (row_index |> float_of_int)
          *. colHeight
          +. (
            row_index
            |> float_of_int !== 0.
        ...
        let pinned =
          column_index
          mod ((col |> int_of_float) - 1) === 0
          || row_index === 0
          || row_index === (row |> int_of_float)
          - 1;
  ...
  let onMouseOver = (event, width, height) => {
    ...
    let mouse =
      Three.newVector2(clientX -. width /. 2., -. (clientY -. height /. 2.));
    Array.mapi(
      (index, vertext: Three.mesh) => {
        ...
        | Some(target) =>
          let dx =
            Three.get_x_vector2(mouse) -. Three.get_x(target.position);
          let dy =
            Three.get_y_vector2(mouse) -. Three.get_y(target.position);
  ...
  let updateFource = () => {
    switch (pointTarget[0]) {
    | Some(target) =>
      Three.set_position_x(
        points[0][target.id].position,
        Three.get_x(force[0]),
      );
      Three.set_position_y(
        points[0][target.id].position,
        Three.get_y(force[0]),
      );
      Three.set_position_z(
        points[0][target.id].position,
        Three.get_z(target.position),
      );
      pointTarget[0] =
        Some({
          id: target.id,
      ...
      (index, networkPoint) =>
        if (!networkPoint.pinned) {
          let positionX =
            ...
            +. (
              Three.get_x(networkPoint.position)
              -. Three.get_x(networkPoint.oldPosition)
              *. resilient
            );
          let positionY =
            ...
            +. (
              Three.get_y(networkPoint.position)
              -. Three.get_y(networkPoint.oldPosition)
              *. resilient
            );
          let positionZ =
            ...
            +. (
              Three.get_z(networkPoint.position)
              -. Three.get_z(networkPoint.oldPosition)
              *. resilient
            );

          Three.set_position_x(
            points[0][index].oldPosition,
            Three.get_x(networkPoint.position),
          );
          Three.set_position_y(
            points[0][index].oldPosition,
  ...
  let updateJoint = () => {
    Array.mapi(
      (index, line) => {
        let dx =
          Three.get_x(line.p2.position) -. Three.get_x(line.p1.position);
        let dy =
          Three.get_y(line.p2.position) -. Three.get_y(line.p1.position);
        let d = Js.Math.sqrt(dx *. dx +. dy *. dy);
        ...
        if (!line.p1.pinned) {
          Three.set_position_x(
            lines[0][index].p1.position,
            Three.get_x(lines[0][index].p1.position) -. offsetX,
          );
          Three.set_position_y(
            lines[0][index].p1.position,
            Three.get_y(lines[0][index].p1.position) -. offsetY,
          );
        };
        if (!line.p2.pinned) {
          Three.set_position_x(
            lines[0][index].p2.position,
            Three.get_x(lines[0][index].p2.position) +. offsetX,
          );
          Three.set_position_y(
            lines[0][index].p2.position,
            Three.get_y(lines[0][index].p2.position) +. offsetY,
          );
        };
      },
      lines[0],
  ...
  [@bs.set] [@bs.scope "geometry"]
  external set_verticesNeedUpdate: (Three.line, bool) => unit =
    "verticesNeedUpdate";
  [@bs.get] [@bs.scope "geometry"]
  external get_verticesNeedUpdate: Three.line => bool = "verticesNeedUpdate";

  [@bs.set]
  external set_lineDistancesNeedUpdate: (Three.line, bool) => unit =
    "lineDistancesNeedUpdate";
  [@bs.get]
  external get_lineDistancesNeedUpdate: Three.line => bool =
    "lineDistancesNeedUpdate";
