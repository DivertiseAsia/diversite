...
module BallWorldScene = {
  ...
  [@bs.send]
  external context_2D: (Dom.element, string) => context2D = "getContext";
  [@bs.send]
  external createRadialGradient:
    (context2D, float, float, float, float, float, float) => gradientColor =
    "createRadialGradient";
  [@bs.set]
  external fillStyle: (context2D, gradientColor) => unit = "fillStyle";
  [@bs.send]
  external fillRect: (context2D, float, float, float, float) => unit =
    "fillRect";
  [@bs.send]
  external addColorStop: (gradientColor, float, string) => unit =
    "addColorStop";
  [@bs.set]
  external set_needsUpdate: (Three.texture, bool) => unit = "needsUpdate";
  ...
  [@bs.get]
  external get_light_position: Three.light => Three.vector3 = "position";
  [@bs.set] [@bs.scope ("shadow", "mapSize")]
  external set_light_width: (Three.light, float) => unit = "width";
  [@bs.set] [@bs.scope ("shadow", "mapSize")]
  external set_light_height: (Three.light, float) => unit = "height";

  [@bs.set] [@bs.scope ("shadow", "camera")]
  external set_light_left: (Three.light, float) => unit = "left";
  [@bs.set] [@bs.scope ("shadow", "camera")]
  external set_light_right: (Three.light, float) => unit = "right";
  [@bs.set] [@bs.scope ("shadow", "camera")]
  external set_light_top: (Three.light, float) => unit = "top";
  [@bs.set] [@bs.scope ("shadow", "camera")]
  external set_light_bottom: (Three.light, float) => unit = "bottom";
  [@bs.set] [@bs.scope ("shadow", "camera")]
  external set_light_far: (Three.light, float) => unit = "far";

  [@bs.set]
  external set_castShadow: (Three.light, bool) => unit = "castShadow";
  ...
  [@bs.send]
  external set_translateX: (Three.mesh, float) => unit = "translateX";
  [@bs.send]
  external set_translateY: (Three.mesh, float) => unit = "translateY";

  let animate = width => {
    for (index in 0 to Array.length(balls[0]) - 1) {
      set_translateX(
        balls[0][index],
        Js.Math.random() *. directions[0][index].x /. 2.,
      );
      set_translateY(
        balls[0][index],
        Js.Math.random() *. directions[0][index].y /. 2.,
      );

      if (Js.Math.abs_float(
            balls[0][index]##position##x
            -. (spheres(widthScene[0], heightScene[0]) |> Array.of_list)[index].
                 x,
          )
          > 20.) {
        let newDirection: Three.vector2 = {
          x: directions[0][index].x *. (-1.),
          y: directions[0][index].y,
        };
        directions[0][index] = newDirection;
      };
      if (Js.Math.abs_float(
            balls[0][index]##position##y
            -. (spheres(widthScene[0], heightScene[0]) |> Array.of_list)[index].
                 y,
          )
          > 20.) {
        let newDirection: Three.vector2 = {
          x: directions[0][index].x,
          y: directions[0][index].y *. (-1.),
        };
        directions[0][index] = newDirection;
      };
    };
    ...
    let targetAngleX =
      mouse[0].x *. Js.Math._PI /. 32. /. (widthScene[0] /. 2.);
    let targetAngleY =
      mouse[0].y *. Js.Math._PI /. 16. /. (widthScene[0] /. 2.);
  ...
  let onMouseMove = (event, width, height) => {
    mouse[0] = {
      x: mouseX(event) -. width /. 2.,
      y: mouseY(event) -. height /. 2.,
    };
  };
};