...
module Document = {
  ...
  [@bs.set]
  external set_mousemove: (Dom.element, ReactEvent.Mouse.t => unit) => unit =
    "onmousemove";
  [@bs.set]
  external set_mouseout: (Dom.element, ReactEvent.Mouse.t => unit) => unit =
    "onmouseout";
...
module Three = {
  ...
  [@bs.new] [@bs.scope "Three"]
  external newMatrix3: unit => matrix3 = "Matrix3";

  [@bs.new] [@bs.scope "Three"]
  external orthographicCamera:
    (float, float, float, float, float, float) => camera =
    "OrthographicCamera";
  [@bs.new] [@bs.scope "Three"]
  external perspectiveCamera: (float, float, float, float) => camera =
    "PerspectiveCamera";
  [@bs.new] [@bs.scope "Three"] external scene: unit => scene = "Scene";
  [@bs.new] [@bs.scope "Three"]
  external newGeometry: unit => geometry = "Geometry";
  [@bs.new] [@bs.scope "Three"]
  external cylinderGeometry: (float, float, float, float) => geometry =
    "CylinderGeometry";
  [@bs.new] [@bs.scope "Three"]
  external sphereGeometry: (float, float, float) => geometry =
    "SphereGeometry";
  [@bs.new] [@bs.scope "Three"]
  external boxGeometry: (float, float, float) => geometry = "BoxGeometry";
  [@bs.new] [@bs.scope "Three"]
  external sphereBufferGeometry: (float, float, float) => geometry =
    "SphereBufferGeometry";
  [@bs.new] [@bs.scope "Three"]
  external planeBufferGeometry: (float, float) => geometry =
    "PlaneBufferGeometry";
  [@bs.new] [@bs.scope "Three"]
  ...
  [@bs.set]
  external set_translate: (geometry, (float, float, float)) => unit =
    "translate";
  ...
  [@bs.set]
  external geometry_set_colors: (geometry, array(color)) => array(color) =
    "colors";
  [@bs.send]
  external geometry_vertices_push: (list(vertice), position) => unit = "push";
  [@bs.send]
  external geometry_vertices_pushs:
    (array(vertice), position, position) => unit =
    "push";
  ...
  let paramAntialias =
    parseIntoParameters("{\"antialias\": true, \"alpha\": true}");
  let renderer = webGLRenderer(paramAntialias);

  let onResize = element =>
    Document.set_onresize(
      Document.bodyElement(Document.doc),
      () => {
        set_aspect(
          camera[0][0],
          Document.offsetWidth(element) /. Document.offsetHeight(element),
        );
