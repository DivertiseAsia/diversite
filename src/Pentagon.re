...
module PentagonScene {
    ...
    let rad_X = [|-.deg2rad(28.), deg2rad(28.), -.deg2rad(38.), deg2rad(48.), -.deg2rad(58.), deg2rad(8.)|];
    let rad_Y = [|deg2rad(10.), deg2rad(20.), deg2rad(20.), -.deg2rad(18.), deg2rad(5.), deg2rad(10.)|];
    let rad_Z = [|deg2rad(30.), deg2rad(15.), -.deg2rad(5.), -.deg2rad(18.), -.deg2rad(30.), deg2rad(10.)|];
    ...
    let particles_body:list(particle) = [
        { x: -. width /. 2. +. 120., y: -.height /. 2. +. 120., z: 0., size: 100., n: 8., o: 0., angle: 0. },
        { x: -. width /. 2. +. 320., y: -.height /. 2. +. 120., z: 0., size: 35., n: 8., o: 0., angle: 0. },
        { x: -. width /. 2. +. 120., y: -.height /. 2. +. 320., z: 0., size: 35., n: 3., o: 0., angle: 0. },
        { x: width /. 2. -. 450., y: -.height /. 2. -. 20., z: 0., size: 50., n: 6., o: 0., angle: 0. },
        { x: width /. 2. -. 200., y: -.height /. 2. +. 50., z: 0., size: 100., n: 6., o: 0., angle: 0. },
        { x: width /. 2. -. 200., y: height /. 2. -. 50., z: 0., size: 100., n: 3., o: 0., angle: 0. },
        { x: width /. 2. -. 550., y: height /. 2. +. 20., z: 0., size: 50., n: 3., o: 0., angle: 0. },
        { x: width /. 2. -. 200., y: height /. 2. -. 250., z: 0., size: 50., n: 8., o: 0., angle: 0. },
    ];
    ...
    let buildParticle = () => {
        let colors = [|[||]|];
        particles_body |> List.map((particle) => {
            ...
            for (x in xStart to xEnd) {
                if (x === validValue[0]) {
                    let axisX = particle.size *. Js.Math.cos((x |> float_of_int) *. Js.Math._PI /. 180.);
                    let axisY = particle.size *. Js.Math.sin((x |> float_of_int) *. Js.Math._PI /. 180.);
                    
                    let newVector3 = Three.newVector3(axisX, axisY, 0.);
                    Three.geometry_vertices_push(Three.geometry_vertices(newGeometry), newVector3);
                    let color = Three.color((x < 180 - 360 / (particle.n |> int_of_float) ? "rgba(255, 252, 249, 1.000)" : "rgba(116, 165, 162, 1.000)"));
            ...
            let params = Three.parseIntoParameters("{\"color\": \"#ffffff\", \"opacity\": 1.0, \"linewidth\": 1.0, \"vertexColors\": 2.0}");
            ...
            let newBody = {
                 ...
                 size: 100., 
                 n: 8., 
                 o: Js.Math.random() /. particle.size *. (Js.Math.random() > 0.5 ? 1. : -1.), 
                 angle: Js.Math.random() *. 360. 
            };
            temp_particles_body[0] = List.append(temp_particles_body[0], [newBody]);
    ...
    let updateParticle = () => {
        ...
        allParticle[0] |> List.mapi((index, particle) => {
            let body = (temp_particles_body[0] |> Array.of_list)[index];
            Three.set_z(particle##rotation, particle##rotation##z +. body.o);
            Three.set_x(particle##position, particle##position##x +. Js.Math.cos(body.angle *. Js.Math._PI /. 180.) *. 0.25);
            Three.set_y(particle##position, particle##position##y +. Js.Math.sin(body.angle *. Js.Math._PI /. 180.) *. 0.25);
            let newBody = {
                x: particle##position##x +. Js.Math.cos(body.angle *. Js.Math._PI /. 180.) *. 0.25, 
                y: particle##position##y +. Js.Math.sin(body.angle *. Js.Math._PI /. 180.) *. 0.25, 
           ...
           updated_particles_body[0] = List.append(updated_particles_body[0], [newBody]);
        }) |> ignore;
    ...
    let buildBackground = () => {
        ...
        let geometry = Three.planeBufferGeometry(Document.windowWidth *. 2., Document.windowHeight *. 2.);
    ...
    let buildPointLight = (color) => {
        let newVector3 = Three.newVector3(Document.windowWidth *. -1., Document.windowHeight, 300.);
    ...
    let getRotationTransformationVector = (x, y, z, size) => {
        ...
        Three.setMatrix3(mX, 1., 0., 0.,
                            0., Js.Math.cos(x), -.Js.Math.sin(x),
                            0., Js.Math.sin(x), Js.Math.cos(x));
        
        Three.setMatrix3(mY, Js.Math.cos(y), 0., Js.Math.sin(y),
                            0., 1., 0.,
                            -.Js.Math.sin(y), 0., Js.Math.cos(y));
    
        Three.setMatrix3(mZ, Js.Math.cos(z), -.Js.Math.sin(z), 0.,
                            Js.Math.sin(z), Js.Math.cos(z), 0.,
                            0., 0., 1.);
    
        let newVector3 = Three.newVector3(0., 0., shift_pivot_z *. size *. radius);
    ...
    let getPivotVector = (obj, first_vertice, second_vertice) => {
        ...
        Three.vector3_copy(vector3_second, obj##geometry##vertices[second_vertice]);
        Three.applyMatrix4(vector3_second, obj##matrix);
        Three.multiplyScalar(Three.addVector3ToVector3(vector3_first, vector3_second), 0.5);
    };
    ...
    let setPosition = (obj, oldPivotVectors, refPivotVectors, x, y, z, size) => {
        let newPosition:Three.position = Three.addVector3ToVector3(
    ...
    let initScene = (camera, renderer, element) => {
        ...
        Three.set_x(pentagon_first##position, -.((Document.offsetWidth(element) /. 1.95) -. radius));
        Three.set_y(pentagon_first##position, Document.offsetHeight(element) /. 5.);
